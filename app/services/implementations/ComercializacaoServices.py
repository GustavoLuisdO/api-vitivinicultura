...
from app.services.interfaces.IComercializacaoServices import IComercializacaoServices
import json


class ComercializacaoServices(IComercializacaoServices):
    async def obter_Comercializacao(self, ano:int):
        try:
            async with httpx.AsyncClient(timeout=120) as client:
                response = await client.get(f"http://vitibrasil.cnpuv.embrapa.br/index.php?ano={ano}&opcao=opt_04")
                ...
                # instanciar o objeto de comercializacao
                comercializacao = Comercializacao(categorias)

                # converter os dados para JSON
                json_str = json.dumps(comercializacao.__json__(), indent=4, ensure_ascii=False)
